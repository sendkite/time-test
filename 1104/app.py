...
def comment_user():
    ...
    db.article.update_one({'idx': int(idx)}, {"$push": {"comment": doc}})
    comment_list = db.article.find_one({'idx': int(idx)}, {'_id': False})

    return jsonify({"result": "success", "msg": "댓글 저장!", "comment_list": comment_list})
